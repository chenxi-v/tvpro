...
import { getEnvConfig, getEnvBool, getEnvNumber } from './runtime-config';
...
async function getInitConfig(configFile: string, subConfig: {
  ...
  const adminConfig: AdminConfig = {
    ...
    SiteConfig: {
      ...
      SearchDownstreamMaxPage: envConfig.SEARCH_MAX_PAGE,
      SiteInterfaceCacheTime: cfgFile.cache_time || 7200,
      DoubanProxyType: envConfig.DOUBAN_PROXY_TYPE,
      DoubanProxy: envConfig.DOUBAN_PROXY,
      DoubanImageProxyType: envConfig.DOUBAN_IMAGE_PROXY_TYPE,
      DoubanImageProxy: envConfig.DOUBAN_IMAGE_PROXY,
      DisableYellowFilter: envConfig.DISABLE_YELLOW_FILTER,
      FluidSearch: envConfig.FLUID_SEARCH,
    },
    UserConfig: {
      Users: [],
    },
  ...
  const allUsers = userNames.filter((u) => u !== envConfig.USERNAME).map((u) => ({
    username: u,
    role: 'user',
    banned: false,
  }));
